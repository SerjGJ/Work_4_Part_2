...
export const App = () => {
	...
	const handleInputChange = (fieldName, value) => {
		...
		if ((fieldName === 'password' || fieldName === 'confirmPassword') && formData.password === value) setFormErrors({ ...formErrors, confirmPassword: '' });
	};
	...
	const isFormValid = () =>
		Object.values(formErrors).every((error) => error === '') && formData.email !== '' && formData.password !== '' && formData.confirmPassword !== '';
		...
		<div className={styles.app}>
			...
			<form onSubmit={handleSubmit}>
				...
				<div>
					...
					<input type="password" value={formData.confirmPassword} onChange={(event) => handleInputChange('confirmPassword', event.target.value)} />
